...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    public class XRLever : XRBaseInteractable
    {
        ...
        [SerializeField]
        [Tooltip("The audio source that will play when the lever is activated")]
        AudioSource m_LeverAudioSource = null;  // AudioSource to control when the lever is activated/deactivated
        ...
        void SetValue(bool isOn, bool forceRotation = false)
        {
            ...
            if (m_Value)
            {
                ...
                PlayAudio(true);  // Play audio when the lever is on
            }
            else
            {
                m_OnLeverDeactivate.Invoke();
                PlayAudio(false); // Stop audio when the lever is off
            }
        ...
        // Method to control the audio based on the lever's state
        void PlayAudio(bool play)
        {
            if (m_LeverAudioSource != null)
            {
                if (play && !m_LeverAudioSource.isPlaying)
                {
                    m_LeverAudioSource.Play();  // Play the audio when the lever is on
                }
                else if (!play && m_LeverAudioSource.isPlaying)
                {
                    m_LeverAudioSource.Stop();  // Stop the audio when the lever is off
                }
            }
        }
